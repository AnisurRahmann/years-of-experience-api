...
export class RenderInterceptor implements NestInterceptor {
    ...
    return next.handle().pipe(
      map((data) => {
        if (req.path.includes('/profile')) {
          // We only want to render the profile page from the server
  ...
  isBrowserRequest(req): boolean {
    ...
    return ['Mozilla', 'Chrome', 'Chromium'].some((browser) =>
  ...
  render(data: Record<string, any>, file: string): string {
    const template = handlebars.compile(
      readFileSync(join(process.cwd(), 'views', file), 'utf8').toString(),
    );
